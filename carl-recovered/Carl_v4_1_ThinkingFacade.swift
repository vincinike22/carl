...
struct ContentView: View {
    ...
    private let muted = Color(red: 108/255, green: 112/255, blue: 115/255)
        ...
        NavigationStack {
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 18) {
                    topBar(title: "Carl", subtitle: "Enter thinking mode") {
                        showingConversationInfo = true
                    }

                    thinkingCueCard(text: "Pause. Write one thought before it is finished.")

                    featureCard {
                        VStack(alignment: .leading, spacing: 14) {
                            labelRow("Write", tone: muted)
                    ...
                    featureCard {
                        VStack(alignment: .leading, spacing: 18) {
                            HStack {
                                labelRow("Conversation", tone: muted)
                                Spacer()
    ...
    private var journalScreen: some View {
        NavigationStack {
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 18) {
                    topBar(title: "Journal", subtitle: "Hold the thought a little longer") {
                        showingJournalInfo = true
                    }

                    thinkingCueCard(text: "Name it gently. The clearer sentence usually comes second.")

                    featureCard {
                        VStack(alignment: .leading, spacing: 14) {
                            labelRow("New Entry", tone: muted)
    ...
    private var journeyScreen: some View {
        NavigationStack {
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 18) {
                    topBar(title: "Journey", subtitle: "See the shape of thought") {
                        showingJourneyInfo = true
                    }

                    thinkingCueCard(text: "Patterns become trustworthy when you can inspect them.")
    ...
    private var reflectionScreen: some View {
        NavigationStack {
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 18) {
                    topBar(title: "Reflection", subtitle: "Let the mind become legible") {
                        showingReflectionInfo = true
                    }

                    thinkingCueCard(text: "Read slower here. The point is not speed, but recognition.")
                    ...
                    featureCard {
                        VStack(alignment: .leading, spacing: 18) {
                            HStack(spacing: 10) {
                                mascotSeal(size: 28)
                                ThinkingGlyph(size: 24, ink: ink, accent: sage)
        ...
        NavigationStack {
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 18) {
                    featureCard {
                        VStack(alignment: .leading, spacing: 12) {
                            labelRow("Import Sources", tone: muted)
    ...
    private var memorySuggestionCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
                mascotSeal(size: 24)
                ThinkingGlyph(size: 20, ink: ink, accent: sage)
    ...
    private func topBar(title: String, subtitle: String, infoAction: @escaping () -> Void) -> some View {
        HStack(alignment: .center) {
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 10) {
                    ThinkingGlyph(size: 22, ink: ink, accent: sage)
    ...
    @ViewBuilder
    private func thinkingCueCard(text: String) -> some View {
        HStack(spacing: 12) {
            ThinkingGlyph(size: 24, ink: ink, accent: sage)
        ...
        .background(
            RoundedRectangle(cornerRadius: 22, style: .continuous)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 22, style: .continuous)
                        .stroke(sage.opacity(0.24), lineWidth: 1)
                )
        )
    }
    ...
    private func conversationBubble(_ message: ConversationItem) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            if message.author == .carl {
                HStack(spacing: 8) {
                    mascotSeal(size: 24)
                    ThinkingGlyph(size: 18, ink: ink, accent: sage)
                    Text("Carl")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
    ...
    private func timelineRow(_ item: JourneyItem) -> some View {
        HStack(alignment: .top, spacing: 14) {
            ...
            featureCard {
                ...
                HStack(spacing: 8) {
                    ThinkingGlyph(size: 18, ink: ink, accent: item.tint.color)
    ...
    private func reflectionBlock(title: String, body: String, tint: Color, serif: Bool = false) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 8) {
                ThinkingGlyph(size: 16, ink: ink, accent: tint)
...
private struct ThinkingGlyph: View {
    let size: CGFloat
    let ink: Color
    let accent: Color
    ...
    var body: some View {
        ZStack {
            ...
            Circle()
                .stroke(accent.opacity(0.45), lineWidth: max(1, size * 0.05))
                .frame(width: size * 0.92, height: size * 0.92)

            Path { path in
                path.move(to: CGPoint(x: size * 0.22, y: size * 0.54))
                path.addCurve(
                    to: CGPoint(x: size * 0.78, y: size * 0.32),
                    control1: CGPoint(x: size * 0.38, y: size * 0.20),
                    control2: CGPoint(x: size * 0.58, y: size * 0.20)
                )
            }
            .stroke(ink.opacity(0.55), style: StrokeStyle(lineWidth: max(1, size * 0.075), lineCap: .round, lineJoin: .round))
...
private struct InfoSheet: View {
    ...
    var body: some View {
        NavigationStack {
            VStack(alignment: .leading, spacing: 16) {
                HStack(spacing: 10) {
                    ThinkingGlyph(size: 24, ink: Color(red: 47/255, green: 52/255, blue: 55/255), accent: Color(red: 183/255, green: 199/255, blue: 176/255))
